...
@register_env("Separate4Instruments-v5", max_episode_steps=500)
class Separate(DefaultCameraEnv):
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        with torch.device(self.device):
            ...
            self.target_obj_idx = torch.full((b,), -1, dtype=torch.long, device=self.device)
    ...
    def evaluate(self):
        ...
        for i, obj in enumerate(self.objects):
            ...
            outside_bin = dist_to_bin > (self.block_half_size[1] + 0.05)

            in_target_area = self._target_in_target_area(target_idx=torch.full((self.num_envs,), i, device=self.device))
    ...
    def compute_dense_reward(self, obs: Any, action: Any, info: dict):
        ...
        active_idx_clamped = target_act_idx.clamp(min=0)
        target_grasped = torch.stack([obs_extra[f"obj{i+1}_grasped"] for i in range(len(self.objects))], dim=1)[batch_idx, active_idx_clamped] > 0.5
        ...
        collision = gripper_touching_bin | gripper_touching_table | (grasped_obj_lifted & grasped_obj_touching_bin)

        target_visible = self.is_object_visible(self.objects[0]) # default fallback
        if has_target.any():
        ...
        not_target_grasped = ~target_grasped
        visibility_reward = (1 - torch.tanh(5 * target_tcp_dist)) * target_visible.float()
        reach_reward = (1 - torch.tanh(5 * target_tcp_dist))
        
        reward += torch.where(not_target_grasped, 0.5 * visibility_reward + reach_reward, torch.zeros_like(reward))

        reward -= 2.5 * collision.float()

        reward += 2.5 * target_grasped.float()
        reward += 2.5 * grasped_obj_lifted.float()
        reward += 2.5 * self._target_in_target_area(active_idx_clamped).float()

        target_obj_positions = torch.stack([obj.pose.p for obj in self.objects], dim=1) 
        target_pos = target_obj_positions[batch_idx, active_idx_clamped]
