...
class Separate(DefaultCameraEnv):
    ...
    def _get_obs_extra(self, info: dict):
        ...
        # Distances of each instrument to the gripper TCP: shape [b, num_instruments]
        ...
        for obj in self.objects:
            ...
            grasped_states.append(is_grasped)
            if is_grasped.any():
                grasped_objs.append(obj)

        grasped_tensor = torch.stack(grasped_states, dim=1)  # [b, num_objects]
        any_grasped = grasped_tensor.any(dim=1)
        ...
        # Distance approximations for contact thresholds
        ...
        gripper_touching_bin = (gripper_to_bin_dist < (self.block_half_size[1] + 0.01)).float() & (self.agent.tcp_pos[..., 2] < 0.1).float()

        # 4. Check if the currently grasped object is touching the bin
        ...
        for i, obj in enumerate(self.objects):
            ...
            # Active if this specific object is grasped AND colliding with the bin boundary
