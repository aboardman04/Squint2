...
class TwoCameraEnv(BaseRandomEnv):
    ...
    def _default_sensor_configs(self):
        ...
        if self.domain_randomization and config.wrist_camera_fov_noise > 0:
            ...
            fov_noise_overhead = config.third_camera_fov_noise * (2 * self._batched_episode_rng.rand() - 1)
        else:
            fov_noise = 0
            fov_noise_overhead = 0

        overhead_pose = sapien.Pose(
            p=self.OVERHEAD_CAMERA_BASE_POS,
            q=qmult(euler2quat(0, self.OVERHEAD_CAMERA_BASE_ROT_RAD[1], self.OVERHEAD_CAMERA_BASE_ROT_RAD[2], axes="rxyz"), 
                    euler2quat(self.OVERHEAD_CAMERA_BASE_ROT_RAD[0], 0, 0, axes="rxyz"))
        )
        ...
        return [
            ...
            CameraConfig(
                ...
                width=128,
                height=128,
                fov=self.OVERHEAD_CAMERA_FOV + fov_noise_overhead,
                near=0.01,
                far=100,
            )
        ]
