...
class Args:
    ...
    wandb_entity: Optional[str] = "aboardman04-rif-robotics"  # CHANGE THIS: your wandb username/entity
    wandb_project: str = "maniskill-so101"  # Your wandb project name
    agent_name: str = "squint"  # Agent name used in wandb
    ...
    if args.checkpoint:
        ...
        if args.checkpoint == "wandb" and args.wandb_seeds:
            seed_to_use = select_best_wandb_seed(
                args.wandb_entity, args.wandb_project, args.agent_name,
    ...
    with KeyboardController() as kb:
        while True:
            ...
            for _ in tqdm(range(args.max_episode_steps), desc="Steps"):
                ...
                if recorder or args.traj_dir:
                    frame = extract_recording_frame(real_obs)
                    if frame is not None:
                        if recorder:
                            recorder.push(frame)
                        traj_rgb.append(frame)
                        traj_qpos.append(real_agent.get_qpos().cpu().numpy().flatten())
