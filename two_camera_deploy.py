...
from deploy_utils.two_camera_robot_config import create_real_robot
from two_camera_train import DeployAgent
...
class Args:
    ...
    env_id: str = "2Camera-SO101LiftCube-v1"
...
def create_wrist_camera_preprocessor(sim_env):
    ...
    def preprocess(sensor_data, sensor_names=None):
        ...
        for sensor_name in sensor_names:
            if sensor_name not in sim_env._sensor_configs:
                continue
...
def overlay_envs(sim_env, real_env):
    ...
    # Sort to ensure consistent order (e.g., base_camera then overhead_camera)
    for name in sorted(sim_obs.keys()):
    ...
    return tile_images(overlaid_imgs), tile_images(real_imgs), tile_images(sim_imgs)
...
def extract_recording_frame(real_obs: dict) -> Optional[np.ndarray]:
    ...
    Looks for the first RGB image in the observation dict. If multiple cameras 
    are concatenated (e.g. 6 channels), it tiles them horizontally.
    ...
    for key in real_obs:
        if "rgb" in key or "image" in key:
            ...
            # Handle concatenated multi-camera images
            channels = img.shape[-1]
            if channels >= 3 and channels % 3 == 0:
                num_cams = channels // 3
                cams = []
                for i in range(num_cams):
                    cam_img = img[..., i*3:(i+1)*3]
